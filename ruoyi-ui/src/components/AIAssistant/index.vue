    ...
    <transition name="slide-up">
      <div v-if="dialogVisible" class="ai-dialog-wrapper">
        ...
        <div class="header-actions">
          <el-tooltip :content="autoPlayVoice ? '关闭自动播放' : '开启自动播放'" placement="left">
            <i 
              :class="autoPlayVoice ? 'el-icon-video-play' : 'el-icon-video-pause'" 
              @click="toggleAutoPlay"
              :style="{ color: autoPlayVoice ? '#67c23a' : '#909399' }"
            ></i>
          </el-tooltip>
...
import { textToSpeech } from '@/api/ai/tts';
...
export default {
  ...
  methods: {
     ...
     * 播放语音
     */
    async playAudio(text, messageIndex) {
      // 如果当前正在播放，先停止
      ...
      try {
        ...
        // 调用TTS API
        const response = await textToSpeech(plainText, 'CHERRY');
        
        console.log('TTS API响应:', response);
        
        if (response.code === 200) {
          // 后端使用AjaxResult.success()返回，数据在msg字段中
          const audioBase64 = response.msg || response.data;
          
          if (!audioBase64) {
            throw new Error('未获取到音频数据');
          }
          
          console.log('✓ 收到Base64音频，长度:', audioBase64.length, '字符');
          
          try {
            // Base64解码
            console.log('开始Base64解码...');
            const binaryString = window.atob(audioBase64);
            console.log('✓ Base64解码成功，二进制长度:', binaryString.length, '字节');
            ...
            console.log('文件头标识:', header, '(应该是RIFF)');
            
            // 创建Blob（使用通用的audio格式，让浏览器自动识别）
            const blob = new Blob([bytes], { type: 'audio/wav' });
            const blobUrl = URL.createObjectURL(blob);
            console.log('✓ 创建Blob URL成功，大小:', bytes.length, '字节');
            ...
            this.currentAudio.onerror = (e) => {
              const err = this.currentAudio.error;
            ...
            this.currentAudio.play()
              ...
              .catch(err => {
                console.error('✗ play()失败:');
                console.error('  错误名称:', err.name);
                console.error('  错误信息:', err.message);
                
                if (err.name === 'NotAllowedError') {
                  console.log('  原因: 浏览器阻止自动播放，需要用户交互');
                  this.$message.warning('请再次点击播放按钮');
                } else if (err.name === 'NotSupportedError') {
                  console.log('  原因: 浏览器不支持此音频格式');
                  this.$message.error('浏览器不支持WAV格式');
                } else {
          ...
          throw new Error(response.msg || '语音合成失败');
        }
      } catch (error) {
